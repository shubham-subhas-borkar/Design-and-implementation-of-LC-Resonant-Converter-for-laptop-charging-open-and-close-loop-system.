...
clear all;
close all;
%% Avergae Current Mode Controller Design for LC Resonant Converter

%% LC series resonant converter - Hard ware Specification
Ls = 12.73e-6;                     % -->Enter Series inductance or Resonant Inductance
Cs = 49.73e-9;                    % -->Enter Resonant capacitance
...
n = 2;                       % -->Enter Transformer turns ration - Np/Ns

%% LC Operating Point
Duty = 0.5;                     % -->Enter Duty ratio
Vin = 40;                     % -->Enter Input voltage
Po_desired = 150;                  % -->Enter Output power
...
Qfac = sqrt(Ls/Cs)/Re;               % Q-factor of LC tank circuit for specific load resistance
...
Gvw = EDF(1);                    % LC converter frequency to output voltage transfer function
Giw = EDF(2);                    % LC converter frequency to tank current transfer function

P = pole(Giw);                   % Extraction of poles of LC converter transfer function                   
[N,K] = zero(Giw);                 % Extraction of zeros of LC converter transfer function 
Giw = minreal(zpk(N,P,K));             % LC converter frequency to output voltage transfer function
...
Vab = 40/2    ;    %nominal input voltage on primary Tx 
ws = 2*pi*fs   ;    % Resonant frequency
Xr = ws*Ls    ;    % Impedance of series Inductance
Xm = ws   ;    % Impedance of magnetizing Inductance
XC = 1/(ws*Cs)  ;    % Impedance of series capacitance
p = (Xr - XC + Xm)/Xm;   % Overall impedance
fr = 200e3    ;    % Resonant frequency
frequencytovoltagefactor = (2*pi^2*fr^3*Cs)/Vab/p^2 ;